...
func GetCalendarSrv() *calendar.Service{
...
func IcalEventsToGcal(calendarSrv *calendar.Service, events []*ical.VEvent) {
	for _, event := range events {
		...
		if err != nil {
			logger.Logger.Errorf("Error parsing start time of event %s, %v\n", eventSummary, err)
			continue
		}

		endTime, err := event.GetEndAt()
		if err != nil {
			logger.Logger.Errorf("Error parsing end time of event %s, %v\n", eventSummary, err)
			continue
		}
		...
		_, err = calendarSrv.Events.Update("primary", gevent.Id, gevent).Do()
		if err != nil {
